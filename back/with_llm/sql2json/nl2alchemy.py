...
def create_sql_to_json_prompt() -> PromptTemplate:
    """JSON Guide 프롬프트 생성"""
    template = """
당신은 SQL을 SQLAlchemy 스타일의 JSON으로 변환하는 전문가입니다. 원래는 두개로 나뉘어져 있던 테이블인데 하나로 합치게 되었습니다.
다음 SQL 쿼리를 합쳐진 원테이블에 대한 결과로 변경해주세요.
주어진 형식의 JSON으로 변환해주세요.
...
def transform_sql_to_json(llm, sql: str) -> Dict[str, Any]:
    ...
    try:
        ...
        schema = SCHEMA
        chain = prompt | llm
        result = chain.invoke({"sql": sql, "schema": schema})
