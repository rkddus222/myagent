...
const LlmPage = () => {
    ...
    if (value.trim()) {
      try {
        ...
        if (jsonData.length === 0) {
          setJsonError('JSON 배열이 비어있습니다.')
          return
        }
        if (!jsonData.every(item => item.question && item.answer)) {
          setJsonError('각 항목은 question과 answer 필드를 포함해야 합니다.')
          return
        }
        setJsonError('')
      } catch (error) {
    ...
    try {
      ...
      if (activeTab === 'multi') {
        try {
          ...
          if (!jsonData.every(item => item.question && item.answer)) {
            throw new Error('각 항목은 question과 answer 필드를 포함해야 합니다.')
          }
          questions = jsonData
        } catch (error) {
          toast.error('JSON 형식이 올바르지 않습니다. 예시: [{"question": "질문1", "answer": "답변1"}]')
          setIsLoading(false)
          return
        }
      ...
      for (const qa of questions) {
        ...
        console.log('Sending request:', requestData)  // 디버깅을 위한 로그 추가
        const cleanAnswer = await respondentApi.convertToSql(requestData)
  ...
  const getResultsAsJson = () => {
    const jsonResults = testResults.map(result => {
      // answer에서 question, answer, stats 부분을 분리
      const [questionText, restText] = result.answer.split('answer:')
      const [answerText, statsText] = (restText || '').split('stats:')
      
      return {
        question: questionText.replace('question:', '').trim(),
        answer: answerText ? answerText.trim() : '',
        stats: statsText ? statsText.trim() : ''
      }
    })
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-6">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-800 dark:text-white mb-2">Respondent 샷 제작</h1>
        </div>
          ...
          <div className="space-y-8">
            {activeTab === 'single' && (
              <div>
                ...
                <textarea
                  className="w-full h-72 p-4 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-700 dark:text-white resize-none shadow-sm"
                  value={question}
            ...
            {activeTab === 'multi' && (
              <div>
                ...
                <textarea
                  ...
                  placeholder='[{"question": "질문1", "answer": "답변1"}, {"question": "질문2", "answer": "답변2"}] 형식으로 입력하세요'
                />
                {jsonError && (
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 transform transition-all duration-300 hover:shadow-2xl">
            ...
            <div className="space-y-6">
              {testResults.map((result, index) => {
                ...
                const [questionText, restText] = result.answer.split('answer:')
                const [answerText, statsText] = (restText || '').split('stats:')
                const parsedQuestion = questionText.replace('question:', '').trim()
                const parsedAnswer = answerText ? answerText.trim() : ''
                const parsedStats = statsText ? statsText.trim() : ''
                  ...
                  <div key={index} className="border border-gray-200 dark:border-gray-700 rounded-xl p-6 hover:shadow-lg transition-all duration-200 bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900">
                    <div className="mb-4">
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <button
                          onClick={() => copyToClipboard(parsedQuestion)}
                      ...
                      <p className="text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm">{parsedQuestion}</p>
                    </div>
                    {parsedAnswer && (
                      <div className="mb-4">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <button
                            onClick={() => copyToClipboard(parsedAnswer)}
                            className="text-blue-500 hover:text-blue-600 text-sm flex items-center transition-colors duration-200"
                          >
                            <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" />
                        ...
                        <p className="text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm whitespace-pre-wrap">{parsedAnswer}</p>
                      </div>
                    )}
                    {parsedStats && (
                      <div className="mb-4">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <button
                            onClick={() => copyToClipboard(parsedStats)}
                            className="text-blue-500 hover:text-blue-600 text-sm flex items-center transition-colors duration-200"
                          >
                            <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 5H6a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2v-1M8 5a2 2 0 002 2h2a2 2 0 002-2M8 5a2 2 0 012-2h2a2 2 0 012 2m0 0h2a2 2 0 012 2v3m2 4H10m0 0l3-3m-3 3l3 3" />
                        ...
                        <pre className="text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm overflow-x-auto">{parsedStats}</pre>
                      </div>
                    )}
